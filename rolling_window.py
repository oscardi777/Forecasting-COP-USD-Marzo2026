...
def cleanup_model_outputs(model_id):
    paths = [
        ...
        os.path.join("resultados_futuro", f"{model_id}_future_predictions.csv"),
    ]
    for p in paths:
        if os.path.isdir(p):
...
def main():
    ...
    for t in range(start_year, end_year + 1):
        ...
        if len(df_train) < SEQ_LEN + 10 or len(df_year) == 0:
            continue
        ...
        for _, pred_len in HORIZONS.items():
            ...
            metrics = compute_metrics_pair(merged[pred_col].values, merged[TARGET].values)
    ...
    print("Rolling window evaluation completed. Results saved in results_rolling_window.csv")
